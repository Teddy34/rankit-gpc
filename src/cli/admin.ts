...
const player = db.select().from(users)
...
if (!shouldBeAdmin) {
  const [{ adminCount }] = db.select({ adminCount: count() }).from(users).where(eq(users.isAdmin, true)).all();
  if (adminCount <= 1) fail("cannot revoke the final administrator.");
}

db.update(users).set({ isAdmin: shouldBeAdmin }).where(eq(users.id, player.id)).run();
