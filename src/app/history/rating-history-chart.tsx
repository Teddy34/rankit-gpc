...
function tickIndexes(gameCount: number): number[] {
  if (gameCount === 0) return [0];
  const count = Math.min(6, gameCount + 1);
  return [...new Set(Array.from({ length: count }, (_, index) => Math.round(index * gameCount / (count - 1))))];
}

export function RatingHistoryChart({ players, gameDates }: { players: RatingHistoryPlayer[]; gameDates: string[] }) {
  ...
  const x = (gameIndex: number) => margin.left + (gameCount ? gameIndex / gameCount : 0) * plotWidth;
  ...
  const xTicks = tickIndexes(gameCount);
  ...
  return <section className="history-layout">
    ...
    <section className="panel history-chart-panel" aria-labelledby="chart-title">
      <div className="panel-heading"><h2 id="chart-title">Elo history</h2><span>{gameCount} game{gameCount === 1 ? "" : "s"}</span></div>
      {gameCount === 0 ? <p className="empty-state">The graph will appear after the first game.</p> : visiblePlayers.length === 0 ? <p className="empty-state">Select a player with game history to show their rating.</p> : <div className="chart-scroll">
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-labelledby="chart-svg-title chart-svg-description">
          ...
          <desc id="chart-svg-description">A line graph showing selected players&apos; Elo ratings after each recorded game.</desc>
          ...
          {xTicks.map((tick) => <g key={tick}><line className="chart-tick" x1={x(tick)} x2={x(tick)} y1={height - margin.bottom} y2={height - margin.bottom + 6} /><text className="chart-axis-label" x={x(tick)} y={height - 22} textAnchor={tick === 0 ? "start" : tick === gameCount ? "end" : "middle"}>{tick === 0 ? gameDates[0] : gameDates[tick - 1]}</text></g>)}
          ...
          {players.map((player, index) => selected.has(player.id) && player.points.length > 0 ? <g key={player.id} style={{ color: colors[index % colors.length] }}>
            <polyline className="rating-line" points={player.points.map((point) => `${x(point.gameIndex)},${y(point.rating)}`).join(" ")} />
            {player.points.map((point, pointIndex) => <circle className="rating-point" key={`${point.gameIndex}-${pointIndex}`} cx={x(point.gameIndex)} cy={y(point.rating)} r="4"><title>{player.displayName}: {point.rating} on {point.playedOn}</title></circle>)}
          </g> : null)}
        </svg>
      </div>}
    </section>
