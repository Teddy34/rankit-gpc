...
import { nowInBrussels, todayInBrussels } from "@/lib/dates";
...
export default async function NewGamePage() {
  ...
  return <main><Link className="back-link" href="/games">← Game history</Link><section className="page-heading"><p className="eyebrow">Rack ’em up</p><h1>Register a game</h1><p>Choose the players, result, and date. Ratings update immediately.</p></section>
    {players.length < 2 ? <div className="notice">At least two active players are needed to register a game.</div> : <GameForm players={players} today={todayInBrussels()} now={nowInBrussels()} defaultPlayerOneId={actor.id} />}
  </main>;
}
